...
function block_theme_setup()
{
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__('Primary', 'block-theme'),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'block_theme_custom_background_args',
			array(
...
/**
	...
	*/
function block_theme_content_width()
{
	$GLOBALS['content_width'] = apply_filters('block_theme_content_width', 640);
}
add_action('after_setup_theme', 'block_theme_content_width', 0);
...
function block_theme_widgets_init()
{
	register_sidebar(
		array(
			'name' => esc_html__('Sidebar', 'block-theme'),
			'id' => 'sidebar-1',
			'description' => esc_html__('Add widgets here.', 'block-theme'),
...
{
	wp_enqueue_style('block-theme-style', get_stylesheet_uri(), array(), _S_VERSION);
	wp_style_add_data('block-theme-style', 'rtl', 'replace');

	wp_enqueue_script('block-theme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);
